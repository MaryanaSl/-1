...
<details>
<summary>Проверка права администрирования</summary>
```bsl
...
В модуле объекта определен обработчик **«ОбработкаЗаполнения»**, в котором заполняетс реквизит **«Тип»** значением по умолчанию **«ЮридическоеЛицо»**, а реквизит **«Ответственный»** — значением параметра сеанса **«ТекущийСотрудник»**.
...
Ниже приведу часть кода из модуля формы элемента справочника Контрагенты:
```bsl
&НаСервере
...
КонецФункции
```
+ реализация проверки корректности ИНН как для физических, так и для юридических лиц. Проверять нужно и при попытке записи — с выдачей предупреждения и отказом, и при изменении ИНН — с подсветкой поля ввода или выводом текста ошибки рядом с ним.
Ниже часть кода из модуля формы элемента справочника Контрагенты:
...
2. Второй интересной задачей тут посчитала создание формы Админиистратора элемента  Справочника "Сотрудники".
На форме, помимо реквизитов сотрудника, должен быть флажок «Вход разрешён» и поля ввода «Имя для входа» и «Пароль», отражающие свойства пользователя ИБ.
+ При записи сотрудника из этой формы при необходимости должно выполняться:
...
В пользовательском режиме форма элемента выглядит так: 
![image](https://github.com/user-attachments/assets/9684e07a-96b4-4500-b792-b40b3dc85101)
